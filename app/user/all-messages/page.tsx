...
export default function AllMessagesPage() {
    ...
    const fetchMessages = useCallback(async () => {
        try {
            ...
            const res: ApiResponse = await fetchApi(`/api/whatsapp/message-list?${qs.toString()}`, {
                method: 'GET',
                headers: { 'Content-Type': 'application/json' }
            });

            // Jika respon kamu masih berupa array (data[0]), sesuaikan:
    ...
    // Fetch ketika page/limit/status berubah, atau search selesai debounce
    useEffect(() => {
        fetchMessages();
    }, [fetchMessages]);

    // Reset ke halaman 1 bila search / status berubah
    useEffect(() => {
        setPage(1);
    }, [debouncedSearch, status, limit]);

    // Re-fetch ketika modal ditutup (misal setelah kirim pesan baru)
    useEffect(() => {
        if (!modal) {
            fetchMessages();
        }
    ...
    return (
        <div className="w-full p-6 bg-white rounded-lg shadow-md">
            ...
            <SendMessageModal open={modal} onClose={() => setModalOpen(false)} />
        </div>
    );
}
...
function StatusBadge({ status }: { status: WhatsAppMessage['status'] }) {
    ...
    return (
        <span
            className={`text-xs font-semibold px-2.5 py-0.5 rounded-full ${map[status]}`}
        >
            {status}
        </span>
    );
...
function Pagination({
    ...
    // Simple windowed pages (ex: show current -2 .. +2)
    const pages = useMemo(() => {
        const window = 2;
...
function Th({ children }: { children: React.ReactNode }) {
    return <th className="p-3 text-xs font-semibold text-gray-600 uppercase tracking-wider">{children}</th>;
}
